...
const[allGastronomyDates, setGastronomyDates] = useState(undefined)
...
console.log(" CONTEXTO de todos los usuarios: ", allUsers)
    ...
    useEffect(() => {
      ...
      const dates = async () => {
          ...
          .then(response => {
            console.log("ALL GASTRONOMY DATES ___>", allGastronomyDates)
            setGastronomyDates(response.data)})
          .catch(err => console.log(err))
          console.log("ANTES DEL BUCLE GASTRONOMY ___>", allGastronomyDates)
            let arrDates = []
          for(let i = 0; i <= allUsers?.length; i++) {
                let [dates]  = allGastronomyDates?.filter( elm => elm.creator === allUsers[i]._id );
                arrDates.push(dates)
            }
